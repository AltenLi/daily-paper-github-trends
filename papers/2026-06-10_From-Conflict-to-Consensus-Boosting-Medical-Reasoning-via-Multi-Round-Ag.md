---
title: "From Conflict to Consensus: Boosting Medical Reasoning via Multi-Round Agentic RAG：从论文到 AI 工程落地的读法"
date: "2026-06-10"
category: "论文精读"
...
- 来源：https://arxiv.org/abs/2603.03292
- arXiv ID：2603.03292
...
- 相关关键词：RAG/检索、Agent/工具调用、评测/Benchmark

这部分是事实层。后面的工程解读会明确区分“论文/摘要中出现的信息”和“我对工程落地的判断”。

## 2. 它真正对应的工程问题

论文摘要里最关键的一点是：At each round, the agent transforms semantic conflict among candidate responses into actionable queries to retrieve external evidence, while optimizing history reasoning traces to mitigate long-context degradation.
...
摘要中关于方法的核心表述是：In this paper, we propose MA-RAG (Multi-Round Agentic RAG), a framework that facilitates test-time scaling for complex medical reasoning by iteratively evolving both external evidence and internal reasoning history within an agentic refinement loop.
...
关于实验/评测，候选记录中的可用信息是：Extensive evaluations across 7 medical Q&A benchmarks show tha
